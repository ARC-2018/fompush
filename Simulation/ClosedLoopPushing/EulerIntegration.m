...
function [x_state, u_c_state, x_bar, u_bar, t, modes, costs] = SimpleIntegrationMPC(obj, t0, tf, x0, x_star, u_star, mpc_solver)
    local_mpc_solver = mpc_solver; % To avoid corrupting the mpc starting data
    ...
    for step = 1:number_of_integration_steps
        ...
        [u_c_state(:, step), modes(step), costs(step), local_mpc_solver] = local_mpc_solver.SolveMPC(current_t, x_star, u_star, x_state(:, step)); % Solve Controller MPC problem and get control input
        u_bar(:, step) = u_c_state(:, step) - u_star(current_t);
        ...
        %Save data 
        plot_data.xs_star_state(step,:) = x_star(t(step))';
        plot_data.us_star_state(step,:) = u_star(t(step));
        [xc_star, uc_star] = mpc_solver.simulator2controller(x_star(t(step)), u_star(t(step)));
